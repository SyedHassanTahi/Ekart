...
const AdminProduct = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const res = await axios.put(`https://ekart-backend-pink.vercel.app/api/v1/product/update/${editProduct._id}`, formData, {
                headers: {
    ...
    const deleteProductHandler = async (productId) => {
        try {
            ...
            const res = await axios.delete(`https://ekart-backend-pink.vercel.app/api/v1/product/delete/${productId}`,
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    }
    ...
    return (
        <div className='pl-89.5 py-25 pr-20 flex flex-col gap-3 min-h-screen bg-gray-100'>
            <div className='flex justify-between'>
                <div className='relative bg-white rounded-lg'>
                    <Input type='text'
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        placeholder='Search Product...' className='w-100 items-center' />
                ...
                <Select onValueChange={(value) => setSortOrder(value)}>
                    ...
                    <SelectContent>
                        <SelectGroup>
                            ...
                            <SelectItem value="highToLigh">Price: High to Low</SelectItem>
                        </SelectGroup>
                    </SelectContent>
                </Select>
            </div>
                ...
                filteredProducts.map((product, index) => {
                    return <Card key={index} className='px-4'>
                        <div className='flex items-center justify-between'>
                            ...
                            text-gray-800'>${product.productPrice}</h1>
                            <div className='flex gap-3'>
                                <Dialog open={open} onOpenChange={setOpen}>
                                    <DialogTrigger asChild>
                                        <Edit onClick={() => { setOpen(true), setEditProduct(product) }} className='text-green-500 cursor-pointer' />
                                    </DialogTrigger>
                                    <DialogContent className="sm:max-w-156.25 max-h-160 overflow-y-scroll">
                                        <DialogHeader>
